...
public class DibujarLinea extends JFrame {
	...
	public DibujarLinea() {
	    ...
	    //JLABELS USADOS PARA DIFERENCIAR ENTRE LA FIGURA ORIGINAL Y LA FIGURA TRANSFORMADA EN LA VENTANA
	...
	void dibujarLasLineas(Graphics g) {
		Scanner entrada = new Scanner(System.in); //SCANNER PARA LEER LOS DATOS INGRESADOS POR TECLADO
		String eleccion;
		System.out.println();
		...
		//SE VALIDA QUE SOLO SE PUEDA INGRESAR UNA DE LAS 3 OPCIONES ESTABLECIDAS
		do {
			System.out.print("a) Tipo de transformación = ");
			eleccion = entrada.next();
			//EN CASO DE QUE NO SE INGRESE UNA DE LAS OPCIONES DEL MENU, VUELVE A PREGUNTAR HASTA QUE SE HAGA
			...
			do {
				...
				if(num_vert==2) { //LINEA
					...
					*/
					...
					//SE GUARDAN LOS DATOS DE LOS NUEVOS VERTICES EN LAS MATRICES
			...
			do {
				...
				if(num_vert==2) { //LINEA
					...
					*/
					...
					//SE GUARDAN LOS DATOS DE LOS NUEVOS VERTICES TRANSFORMADOS (RESOLVIENDO LA MULTIPLICACION MATRICIAL)
					double x1_prima = y1; // (x1*0) + (y1*1)
					double y1_prima = x1; // (x1*1) + (y1*0)
					double x2_prima = y2; // (x2*0) + (y2*1)
					double y2_prima = x2; // (x2*1) + (y2*0)
				...
				if(num_vert==3) { //TRIANGULO
					...
					*/
					...
					double x2_prima = y2; //(x2*0) + (y2*1)
					double y2_prima = x2; //(x2*1) + (y2*0)
					double x3_prima = y3; //(x3*0) + (y3*1)
					double y3_prima = x3; //(x3*1) + (y3*0)
				...
				if(num_vert==4) { //FIGURAS DE 4 LADOS (CUADRADO, RECTANGULO, ROMBO, ROMBOIDE, ETC)
					...
					*/
					...
					double x3_prima = y3; //(x3*0) + (y3*1);
					double y3_prima = x3; //(x3*1) + (y3*0);
					double x4_prima = y4; //(x4*0) + (y4*1);
					double y4_prima = x4; //(x4*1) + (y4*0);
...
//PROYECTO #2 - TRANSFORMACIONES 2D
//INTEGRANTES
//GARCÍA AGUILAR JOSÉ ALFREDO
//MORENO PEREZ JOHANA GISELLE
//GRAFICACIÓN, CLASE 7-8 AM